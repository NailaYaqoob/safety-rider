...
### Shade, from satellite segmentation — built, and gated by plan tier

> **Not active on this API key.** `POST /v1/satellite` is Premium-only, and
> this project runs on a **Hackathon** plan. Three calls were made to confirm
> it, including the exact request the quickstart documents as working, against
> the quickstart's own Chicago point — all three returned `status: Failed`.
> With no segmentation available, routes rank on measured temperature alone,
> exactly as they did before this was written. Nothing degrades; the shade line
> simply never appears.
>
> What follows describes a mechanism that is implemented and tested against
> real segmentation responses, not a feature running in the demo. It is here
> because it is one endpoint away from working, not because it is working.
...
each cell along a route can feed the ranking:
...
The rider text above is what the code produces when segmentation is available —
it is generated by the test suite, not transcribed from a live run.
...
like the nowcast. Land cover does not change, so a cell would be paid for once
and cached with no expiry — `python -m safety_rider.warm 33.4484 -112.0740
--shade-only`, or automatically for every cell in `SAFETY_RIDER_WARM_CELLS`.
On a Premium key that is the whole of the setup; on this one it returns
`Failed` and the router carries on without it.

The vocabulary work was worth doing regardless. The four real segmentation
responses cached in `data/satellite/` come from the upstream template, and
parsing them showed the first version of this code wrong in three ways — a
reminder that a schema guessed from prose is a guess, and that the cheapest
place to find that out is a fixture rather than a live call.
...
- **Shade routing needs a Premium key.** The canopy scoring is implemented and
  tested, but `/v1/satellite` is not available on the Hackathon plan this runs
  on, so no route in the live demo is ranked by shade. Confirmed by three
  failed calls, one of them the quickstart's own documented example. See
  [Shade, from satellite segmentation](#shade-from-satellite-segmentation--built-and-gated-by-plan-tier).
